...
def MuLawEncode(data):
    size = int ((len(data) / 2))
    encoded = bytearray()
    for x in range(size):
        encoded.append(u_law_e((data[2 * x + 1] << 8) | data[2 * x]))
    return encoded

# def MuLawDecode(data):
#
#     size = len(data);
#     decoded =bytearray();
#     for x in range(size):
#         #//First byte is the less significant byte
#         decoded[2 *x] = ([data[x]] & 0xff);
#         #//Second byte is the more significant byte
#         decoded[2 * x + 1] = (muLawToPcmMap[data[x]] >> 8);
#
#     return decoded
...
if __name__ == '__main__':
   # a= u_law_e(-1)
   # b=u_law_e(0)
   # print(least_sagnificant_bit(100))
   # print(most_sagnificant_bit(50))
   ...
   # print(u_law_d(a))
   # print(u_law_d(b))
   # response = requests.get("https://omt-hq.sunstone-cloud.com/tagdata/history?tagId=65501&property_equal=accelerometer&timeformat=msFromEpoch&from_msFromEpoch=-60000")  # Send the request and recieve the response
   # data = response.json()  # convert the response body into JSON
   # accX=[]
   # accY=[]
   # accZ=[]
   # for x in range(len(data)):
   #     accX.append(int(round((data[x]['value'][0] * 1000))))
   #     accY.append( int(round((data[x]['value'][1] * 1000))))
   #     accZ.append( int(round((data[x]['value'][2] * 1000))))
   #
   # print(accZ)
   # for x in range(len(accZ)):
   #     print('normal :\t'+ str(accZ[x]) + " Encoded :"+ str(u_law_e(accZ[x])) + "bits: " + str(bytes(u_law_e(accZ[x]))) )
   # for x in range(100):
   a = 8159
   b = u_law_e(a)
   decoded=u_law_d(b)
   print("Number: %s  || Binary : %s || Encoded  %s ||binary  %s || decoded  %s ||binary  %s" % (a,bin(a), b,bin(b),decoded,bin(decoded)))
